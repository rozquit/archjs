...
const onAbortedOrFinishedResponse = (res, readStream) => {
  if (res.id === -1) {
    console.log('[archjs | streaming]', ['ERROR! onAbortedOrFinishedResponse called twice for the same res!'])
  } else {
    console.log('[archjs | streaming]', [`stream was closed, openStreams: ${openStreams}`])
    readStream.destroy()
  }
